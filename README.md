...
**URL de production (serveur WebSocket) :** https://vibe-io-server.onrender.com _(Render, free tier — peut prendre ~30-50s à se réveiller après une période d'inactivité)_
...
- **Test instable, corrigé deux fois — le premier correctif traitait le symptôme, pas la cause.** Le test "un joueur plus gros mange un joueur plus petit" échouait par intermittence : un pellet bonus choisi aléatoirement pour faire grandir le joueur pouvait se trouver à quelques pixels d'un pellet malus, annulant le gain de score. Premier correctif de l'IA : un helper `findIsolatedBonusPellet` garantissant l'absence de pellet voisin — 8 runs locaux au vert, poussé en CI avec confiance. **La CI a quand même échoué dès le premier run réel**, sur un test différent ("deux joueurs égaux ne se mangent pas") que l'IA pensait non concerné. En creusant, la vraie cause était plus profonde que le premier diagnostic : `GameRoom.update()` résout les collisions de TOUS les joueurs à chaque tick, y compris ceux qu'on ne déplace pas intentionnellement dans un test — si le spawn aléatoire d'un joueur tombe par hasard près d'un pellet, il le mange dès le premier `update()` de la séquence, même si le test ne testait "que" un autre joueur. Le premier correctif ne couvrait que la destination d'un déplacement, pas les positions de spawn initiales. Correction définitive : un helper `createRoomWithPlayersAwayFromPellets` qui retente la création de la room jusqu'à ce que tous les joueurs spawnent loin de tout pellet — validé sur 150 exécutions consécutives sans échec avant de repousser. Le vrai enseignement : un test vert en local plusieurs fois de suite ne prouve pas l'absence de hasard mal maîtrisé, et la CI a immédiatement révélé ce qu'une vérification locale, même répétée, avait laissé passer.
...
- **Serveur qui ne répondait à aucune requête HTTP simple.** Une fois déployé sur Render, le serveur restait introuvable : `curl` se connectait (TLS OK, requête envoyée) mais n'obtenait jamais de réponse, juste un timeout. Cause réelle : `createServer()` était appelé sans aucun handler, et Socket.io n'intercepte que les requêtes vers `/socket.io/*` — toute autre requête (dont les health checks de Render) tombait dans le vide puisqu'aucun listener ne répondait jamais. Le jeu aurait probablement fonctionné quand même côté WebSocket, mais Render n'aurait jamais pu confirmer que le service était sain. Corrigé en ajoutant un handler HTTP minimal (JSON de statut) *avant* d'attacher Socket.io au serveur — un bug qui ne se voit qu'en testant le déploiement réel, pas en local où on ne tape jamais l'URL racine au hasard.
